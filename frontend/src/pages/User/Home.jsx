...
export default function Home() {
  ...
  const detectLocation = () => {
    if (!navigator.geolocation) { toast.error(t('error')); return; }
    setLocLoading(true);
    navigator.geolocation.getCurrentPosition(
      ({ coords: c }) => {
        setUserCoords({ lat: c.latitude, lng: c.longitude });
  ...
  return (
    <div>
      ...
      <div className="bg-gradient-to-br from-green-800 via-green-700 to-amber-800 rounded-2xl p-4 sm:p-7 mb-6 text-white relative overflow-hidden">
        ...
        <h1 className="text-xl sm:text-3xl font-bold mb-1 relative">{t('freshToddy')}</h1>
        <p className="text-green-200 text-sm mb-3 relative">{t('tagline')}</p>
        {locMsg && (
          <div className="flex flex-wrap items-center gap-2 mb-3">
            <span className="text-green-300 text-xs">{locMsg}</span>
            {userCoords && <span className="text-green-400 text-xs">{t('sortedByDistance')}</span>}
          </div>
        )}
      ...
      {loading ? <Spinner size="lg" /> : (
        <>
          <p className="text-sm text-gray-500 mb-4">
            {shops.length === 0
              ? t('noShopsFound')
              : `${shops.length} ${shops.length !== 1 ? t('shopsAvailable') : t('shopAvailable')}`}
          </p>

          {shops.length === 0 ? (
            <div className="text-center py-16 text-gray-400">
              <div className="text-6xl mb-3">🌴</div>
              <p className="text-lg font-medium">{t('noShopsAvailable')}</p>
                ...
                <Link to={`/shop/${shop._id}`} key={shop._id}
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex gap-3 text-sm">
                      ...
                      <span className="flex items-center gap-1 text-gray-600">🕒 {shop.deliveryTime} {t('minDelivery')}</span>
                    </div>
                    <div className="mt-2 pt-2 border-t border-gray-100">
                      {shop.deliveryCharge > 0
                        ? <p className="text-xs text-gray-500">{t('delivery')}: ₹{shop.deliveryCharge}</p>
                        : <p className="text-xs text-green-600 font-semibold">{t('freeDelivery')}</p>}
                    </div>
                  </div>
                </Link>
              ))}
